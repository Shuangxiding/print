<template>
  <div class="page">
    ...
    <div class="header">
      <el-row>
        <el-col :span="8"> 逾期还款应还总金额:
          <span>{{(caseInfo.cupoAmt).toFixed(2)}}元</span></el-col>
        <el-col :span="8">
          已还款金额：
          <span>{{(caseInfo.cupoPaynum).toFixed(2)}}元</span>
        </el-col>
        <el-col :span="8">
          剩余应还金额：
          <span>{{(caseInfo.cupoAmt-caseInfo.cupoPaynum).toFixed(2)}}</span>
        </el-col>
      </el-row>
    </div>
    <data-grid class="data-grid" :labelWidth="100" labelAlign="center" contentAlign="left">
      <data-grid-item label="还款类型:" :span="12">
        <el-radio v-for="item of $helper.getTypeCode('0032')" :key="item.id" v-model="repayType" :label="item.id">{{item.name}}</el-radio>
      </data-grid-item>
      <data-grid-item label="减免金额:" :span="12" v-show="repayType===132">
        <el-row>
          <el-col :span="8">
            <el-input v-model="derateFee" placeholder="请输入减免金额" type="number" @change="degreeAmt" @blur="derate"></el-input>
          </el-col>
      ...
      <data-grid-item label="减免费用备注:" :span="12" v-show="repayType===132">
        <el-input v-model="derateDescripton" type="textarea" placeholder="请输入内容"></el-input>
      </data-grid-item>
      <data-grid-item label="还款金额:" :span="12">
        <el-row>
          <el-col :span="8">
            <el-input v-model="repayAmount" placeholder="请输入内容" type="number" :readonly="repayType===131||repayType===132" @blur="repay"></el-input>
          </el-col>
          <el-col :span="1" :offset="1">
            <span style="line-height:30px;">元</span>
          </el-col>
...
<script>
  ...
  export default {
    ...
    watch: {
      repayType() {
        if (this.repayType === 131) {
          this.repayAmount = (this.caseInfo.cupoAmt).toFixed(2)
        } else {
          this.repayAmount = ''
        }
      }
    ...
    methods: {
       ...
       */
      degreeAmt() {
        if ((this.repayType === 132) && (this.derateFee > 0)) {
          this.repayAmount = (this.caseInfo.cupoAmt - this.derateFee).toFixed(2)
          if (this.derateFee > this.caseInfo.cupoAmt) {
            this.derateFee = (this.caseInfo.cupoAmt).toFixed(2)
            this.repayAmount = 0
          }
        }
      },
