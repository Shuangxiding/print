<template>
  <div>
    <el-col class="frame">
      <data-grid label="开户信息" iconClass="icon-kaihuxinxi" :labelWidth="100" labelAlign="right" row-key="orapId" contentAlign="left">
      ...
      <data-grid row-key="Id" :totalSpan="10" label="客户基本信息：" iconClass="icon-kehujibenziliao" :labelWidth="100" labelAlign="right" contentAlign="left">
      ...
      <el-col class="text-left data-grid-title" style="float:none">
        <span class="iconfont icon-chechan"></span>
        <span>车产信息</span>
      </el-col>
      <data-grid :totalSpan="10 " v-if="params.customerVehicles.length===0 " iconClass="icon " :labelWidth="100 " labelAlign="right " contentAlign="left ">
      ...
      <data-grid label="单位信息 " iconClass="icon-danweixinxi " :labelWidth="100 " labelAlign="right " contentAlign="left ">
      ...
      <data-grid label="收支状况 " iconClass="icon-shouzhizhuangkuang" :labelWidth="100 " labelAlign="right " contentAlign="left ">
      ...
      <el-col class="text-left data-grid-title" style="float:none">
        <span class="iconfont icon-lianxirenxinxi"></span>
        <span>联系人信息</span>
      </el-col>
      <el-table :data="params.customerContacts">
        ...
        <el-table-column prop="contactIDCard" label="身份证号" min-width="180">
        </el-table-column>
        <el-table-column prop="contactPhone" label="手机号" min-width="130">
        </el-table-column>
        <el-table-column prop="contactInfoTelNo" label="固定电话" min-width="150">
        </el-table-column>
        <el-table-column prop="contactInfoIsKnow" label="是否知晓此项借款(是/否)" min-width="180">
          <template scope="scope">
            <span>{{scope.row.contactInfoIsKnow | knowType}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="contactEmployer" label="工作单位" min-width="150">
        </el-table-column>
      </el-table>
    </el-col>
  </div>
  ...
  export default {
    ...
    methods: {
      refresh(cust) {
        ...
        if (cust.type === 'IMPORT') {
          custServer = {
            server: server.CaseManage.import_case_info_details,
            data: {
              // batchNumber: cust.batchNumber,
          ...
          custServer = {
            server: server.CaseManage.get_case_info_details,
            data: {
              // prinCode: cust.prinCode,
              // idCardNumber: cust.idCardNumber
              custId: cust.custId
            }
          }
        }
        net.send(custServer).then((data) => {
          console.log(data)
          if (!data) {
            this.params = {
              accountType: '',
